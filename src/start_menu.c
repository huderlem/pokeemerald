...
static bool8 StartMenuComfyAnimsCallback(void);
...
static const struct MenuAction sStartMenuItems[] =
{
    ...
    [MENU_ACTION_COMFY_ANIMS]     = {gText_ComfyAnimsMenu, {.u8_void = StartMenuComfyAnimsCallback}},
};
...
static void BuildNormalStartMenu(void)
{
    if (TRUE || FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE)
    {
        AddStartMenuAction(MENU_ACTION_POKEDEX);
    }
    if (TRUE || FlagGet(FLAG_SYS_POKEMON_GET) == TRUE)
    {
    ...
    if (TRUE || FlagGet(FLAG_SYS_POKENAV_GET) == TRUE)
    {
    ...
    AddStartMenuAction(MENU_ACTION_COMFY_ANIMS);
}
...
static bool8 StartMenuComfyAnimsCallback(void)
{
    if (!gPaletteFade.active)
    {
        ...
        RemoveExtraStartMenuWindows();
        CleanupOverworldWindowsAndTilemaps();
        SetMainCallback2(CB2_InitComfyAnimScreen);
        gMain.savedCallback = CB2_ReturnToFieldWithOpenMenu;
